...
fn parse_coords_str(coords: &str, line: String) -> Result<[u32; 3], D22Error> {
    coords
        .split(',')
        .map(|s| s.parse::<u32>())
...
fn set_block_fall(b: &([u32; 3], [u32; 3]), fall: &mut Vec<Vec<Vec<u32>>>, fill_value: u32) {
...
fn solve(fp: &str) -> Result<u32, D22Error> {
    ...
    let mut disintegratable: HashSet<_> = (0..blocks.len()).collect();
